...
import { markdownToHTML } from "@raven-js/beak/md";
...
async function setupRoutes() {
  try {
    ...
    const routes = await registerFileRoutes(router, "src/pages", {
      ...
      indexFile: "index.js",
      includeNested: true,
      handler: async (/** @type {import("@raven-js/wings").Context} */ ctx, route) => {
        /** @type {PageModule} */
        const pageModule = await import(route.module);

        // 1. Load dynamic data if loader exists
        const data = pageModule.loadDynamicData ? await pageModule.loadDynamicData(ctx) : {};

        // 2. Resolve each export (static string OR async function)
        const title = typeof pageModule.title === "function" ? await pageModule.title(data) : pageModule.title;

        const description =
          typeof pageModule.description === "function" ? await pageModule.description(data) : pageModule.description;

        const body = typeof pageModule.body === "function" ? await pageModule.body(data) : pageModule.body;
