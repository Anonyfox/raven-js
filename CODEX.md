...
**Libraries** are runtime dependencies—packages intended for npm installation directly into application codebases. These maintain zero runtime dependencies as unbreakable guarantee, with carefully crafted sub-exports enabling optimal tree-shaking while grouping related functionality sensibly. Libraries integrate seamlessly into existing projects without introducing external attack vectors or dependency conflicts.

**Tools** are CLI executables—tools executed via npx to achieve specific outcomes like building, testing, or deployment. These packages accept strictly necessary external dependencies since they operate outside application runtime, but default to zero dependencies whenever possible. Tools provide surgical solutions for development workflow challenges without polluting application dependency graphs.

**Platform** encompasses framework foundation and ecosystem management—the glue and infrastructure that makes the ecosystem feel seamless. These serve as coordination tools for the murder's collective development efforts and provide the foundation for complete applications.
