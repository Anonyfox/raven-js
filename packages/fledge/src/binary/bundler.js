...
export class BinaryBundler {
  ...
  async generate(outputDir) {
    ...
    try {
      ...
      this.#statistics.totalTime = this.#statistics.endTime - this.#statistics.startTime;
  ...
  async #generateAssetIndex() {
    ...
    for (const filePath of assetFiles) {
      ...
      try {
        ...
        // Determine mount path - use mount map if available, otherwise fallback to full path
        let mountPath = filePath;
        for (const [mountKey, fullPath] of mountMap.entries()) {
        ...
        if (this.#isTextFile(filePath)) {
          this.#assetContent[mountPath] = content.toString("utf8");
        } else {
          this.#assetContent[mountPath] = content.toString("base64");
        }
      } catch (error) {
        const err = /** @type {Error} */ (error);
  ...
  async #generateServerBundle() {
    ...
    try {
      ...
      const finalServerCode = [envCode, serverCode].filter(Boolean).join("\n\n");
  ...
  #generateSeaConfig() {
    ...
    const seaAssets = {};
    for (const [assetPath, contentOrPath] of Object.entries(this.#assetContent)) {
      if (assetPath === "@raven-js/assets.json") {
        ...
        const tempAssetPath = resolve(`${this.#assetIndexPath}..${assetPath.replace(/[/\\]/g, "_")}`);
    ...
    const seaConfig = {
      ...
      disableExperimentalSEAWarning: /** @type {any} */ (seaOptions).disableExperimentalSEAWarning,
  ...
  async #injectSeaBlob() {
    ...
    await inject(/** @type {string} */ (this.#executablePath), "NODE_SEA_BLOB", blobContents, {
