...
describe("generateBinaryExecutable", () => {
	...
	it("throws error for invalid configuration content", async () => {
		...
		await assert.rejects(
			async () =>
				await generateBinaryExecutable(invalidConfig, { validate: true }),
			{
				name: "Error",
				message: /Failed to parse configuration/,
			},
...
describe("parseConfigInput integration", () => {
	it("handles edge case configuration inputs", async () => {
		...
		await assert.rejects(
			async () => await generateBinaryExecutable("", { validate: true }),
			{
				name: "Error",
				message: /Failed to parse configuration/,
			},
		...
		await assert.rejects(
			async () =>
				await generateBinaryExecutable({ invalid: true }, { validate: true }),
			{
				name: "Error",
				message: /Failed to parse configuration/,
			},
