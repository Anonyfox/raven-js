...
describe("Bundler", () => {
	...
	describe("generate", () => {
		...
		it("generates executable with assets", async () => {
			...
			assert.ok(executable.includes(JSON.stringify(textAsset)));
			assert.ok(executable.includes(JSON.stringify(binaryAsset)));
			assert.ok(executable.includes("test"));
			// Binary data should be base64 encoded, not raw
			const expectedBase64 = Buffer.from("fake-png-data", "utf8").toString(
				"base64",
			);
		...
		it("generates executable with custom Node flags", async () => {
			...
			assert.ok(
				executable.startsWith(
					"#!/usr/bin/env node --experimental-sqlite --max-old-space-size=4096 --trace-warnings",
				),
			);
		});
