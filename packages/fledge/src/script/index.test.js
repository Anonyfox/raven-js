...
describe("generateScriptBundle", () => {
  ...
  describe("config input handling", () => {
    ...
    it("throws error for missing config file", async () => {
      await assert.rejects(async () => await generateScriptBundle("./nonexistent.js"), {
        name: "Error",
        message: /Failed to parse configuration/,
      });
    });
  ...
  describe("file writing", () => {
    it("writes executable file by default", async () => {
      ...
      assert.ok(readFileSync(outputPath, "utf8").includes("Hello from script bundle!"));
    });
    ...
    it("creates output directory if it doesn't exist (mkdirp behavior)", async () => {
      ...
      const outputPath = join(missingDir, "created-app.js");
      const configObject = {
        entry: entryFile,
        output: outputPath,
      };
