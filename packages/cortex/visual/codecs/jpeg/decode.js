 ...
 *
 * Uses jpeg-js library implementation for robust JPEG decoding.
 * Adapted to match RavenJS expected interface and return format.
 */
...
export async function decodeJPEG(buffer) {
  ...
  try {
    // Use jpeg-js decode function
    const decoded = decodeJpegJs(rawData);

    // Extract metadata
    const metadata = {
      progressive: decoded.jfif ? false : true, // Simplified detection
      precision: 8, // Standard JPEG precision
      components: decoded.components ? decoded.components.length : 3,
      quality: "unknown",
...
// JPEG-JS Implementation (adapted from https://github.com/eugeneware/jpeg-js)
...
function decodeJpegJs(jpegData, opts = {}) {
  const defaultOpts = {
  ...
  decoder.opts = options;
  JpegImage.resetMaxMemoryUsage(options.maxMemoryUsageInMB * 1024 * 1024);
  decoder.parse(arr);

  const channels = options.formatAsRGBA ? 4 : 3;
  ...
  try {
    ...
    image = {
      ...
      data: options.useTArray ? new Uint8Array(bytesNeeded) : new Uint8Array(bytesNeeded),
    };
    if (decoder.comments && decoder.comments.length > 0) {
  ...
  decoder.copyToImageData(image, options.formatAsRGBA);
  image.jfif = decoder.jfif;
  image.adobe = decoder.adobe;
  return image;
}
...
function JpegImage() {
  ...
  this.parse = function parse(data) {
    ...
    function prepareComponents(frame) {
      ...
      for (componentId in frame.components) {
        if (Object.hasOwn(frame.components, componentId)) {
  ...
  this.copyToImageData = function copyToImageData(imageData, formatAsRGBA) {
  ...
  this.getData = function getData(width, height) {
        ...
        for (y = 0; y < height; y++) {
          ...
          for (x = 0; x < width; x++) {
            if (!colorTransform) {
              ...
              G = component2Line[0 | (x * component2.scaleY * scaleY)];
...
JpegImage.resetMaxMemoryUsage = (maxMemoryUsageBytes_) => {
  totalBytesAllocated = 0;
  maxMemoryUsageBytes = maxMemoryUsageBytes_;
};

JpegImage.getBytesAllocated = () => totalBytesAllocated;

JpegImage.requestMemoryAllocation = requestMemoryAllocation;
