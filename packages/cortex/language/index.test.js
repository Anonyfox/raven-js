...
describe("language module", () => {
  it("re-exports all analysis functions", () => {
    ...
    assert.ok(typeof language.analyzeAITransitionPhrases === "function", "Should export analyzeAITransitionPhrases");
    assert.ok(typeof language.analyzeNgramRepetition === "function", "Should export analyzeNgramRepetition");
    assert.ok(typeof language.analyzeZipfDeviation === "function", "Should export analyzeZipfDeviation");
    assert.ok(typeof language.approximatePerplexity === "function", "Should export approximatePerplexity");
    assert.ok(typeof language.calculateBurstiness === "function", "Should export calculateBurstiness");
    assert.ok(typeof language.calculateShannonEntropy === "function", "Should export calculateShannonEntropy");
    assert.ok(typeof language.detectEmDashEpidemic === "function", "Should export detectEmDashEpidemic");
    assert.ok(
      typeof language.detectParticipalPhraseFormula === "function",
      "Should export detectParticipalPhraseFormula"
    );
    assert.ok(typeof language.detectPerfectGrammar === "function", "Should export detectPerfectGrammar");
    assert.ok(typeof language.detectRuleOfThreeObsession === "function", "Should export detectRuleOfThreeObsession");
  });

  it("re-exports transformation functions", () => {
    // Verify stemming functions are available
    assert.ok(typeof language.stemPorter2 === "function", "Should export stemPorter2");
    assert.ok(typeof language.stemCistem === "function", "Should export stemCistem");
  });
  ...
  it("re-exports featurization functions", () => {
    ...
    assert.ok(typeof language.hashFeatures === "function", "Should export hashFeatures");
    assert.ok(typeof language.rake === "function", "Should export rake");
    assert.ok(typeof language.textrank === "function", "Should export textrank");

    // Similarity functions
    assert.ok(typeof language.osaDistance === "function", "Should export osaDistance");
    assert.ok(typeof language.osaSimilarity === "function", "Should export osaSimilarity");
    assert.ok(typeof language.jaroSimilarity === "function", "Should export jaroSimilarity");
    assert.ok(typeof language.jaroWinklerSimilarity === "function", "Should export jaroWinklerSimilarity");
    assert.ok(typeof language.findBestMatch === "function", "Should export findBestMatch");
    assert.ok(typeof language.groupSimilarStrings === "function", "Should export groupSimilarStrings");

    // MinHash class
    assert.ok(typeof language.MinHasher === "function", "Should export MinHasher class");

    // LSH class
    assert.ok(typeof language.LSHBuckets === "function", "Should export LSHBuckets class");

    // SimHash class
    assert.ok(typeof language.SimHasher === "function", "Should export SimHasher class");
  });
  ...
  it("functions are callable", () => {
    ...
    assert.ok(typeof burstinessResult === "number", "calculateBurstiness should return a number");
    ...
    assert.ok(typeof entropyResult === "number", "calculateShannonEntropy should return a number");
  ...
  it("transformation functions are callable", () => {
    ...
    assert.strictEqual(englishResult, "run", "Should stem English words correctly");

    const complexEnglish = language.stemPorter2("nationalization");
    assert.strictEqual(complexEnglish, "nation", "Should handle complex English suffixes");

    // Test German stemming
    const germanResult = language.stemCistem("laufen");
    assert.strictEqual(germanResult, "lauf", "Should stem German words correctly");

    const germanUmlaut = language.stemCistem("Möglichkeit");
    assert.strictEqual(germanUmlaut, "moeglich", "Should handle German umlauts and suffixes");
  });
  ...
  it("featurization functions are callable", () => {
    ...
    assert.ok(Array.isArray(wordNgrams), "Should return word n-grams array by default");
    assert.ok(wordNgrams.includes("machine learning"), "Should contain expected bigram");
    assert.ok(wordNgrams.includes("learning algorithms"), "Should contain expected bigram");

    // Test character n-grams
    const charNgrams = language.ngrams(testText, { type: "chars", n: 3 });
    assert.ok(Array.isArray(charNgrams), "Should return character n-grams array");
    ...
    assert.ok(typeof mixedNgrams === "object", "Should return mixed features object");
    assert.ok(Array.isArray(mixedNgrams.char), "Should have character features");
