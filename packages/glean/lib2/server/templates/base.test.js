...
describe("baseTemplate", () => {
	...
	test("constructs proper page title", () => {
		const html = baseTemplate(minimalOptions);
		assert(
			html.includes("<title>Test Page - test-package Documentation</title>"),
	...
	test("includes navigation structure", () => {
		...
		assert(
			html.includes('href="/api/">API Reference</a>'),
			"Contains API Reference link",
		);
	});
	...
	test("includes search form when enabled", () => {
		const htmlWithSearch = baseTemplate({
			...minimalOptions,
			navigation: { showSearch: true },
		});

		assert(
			htmlWithSearch.includes('<form class="d-flex" role="search">'),
			"Contains search form",
		);
		assert(
			htmlWithSearch.includes('placeholder="Search documentation..."'),
	...
	test("includes footer with generation info", () => {
		...
		assert(html.includes("Generated by"), "Contains generation info");
		assert(html.includes("Glean"), "Contains Glean reference");
		assert(html.includes("RavenJS toolkit"), "Contains RavenJS reference");
		assert(html.includes("Last updated:"), "Contains last updated");
	});
	...
	test("accessibility features included", () => {
		...
		assert(htmlWithSearch.includes("aria-label"), "Contains aria labels");
		assert(htmlWithSearch.includes('role="search"'), "Contains search role");
