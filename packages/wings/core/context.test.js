...
describe("Context", () => {
  ...
  describe("constructor", () => {
    it("should create instance and validate parameters", () => {
      ...
      assert.equal(ctx.protocol, "https");
  ...
  describe("response shortcuts", () => {
    it("should set all response types", async () => {
      ...
      assert.equal(ctx.responseHeaders.get("content-type"), "application/javascript");
  ...
  describe("edge cases", () => {
    ...
    it("should handle body parsing edge cases", () => {
      ...
      const specialJsonBody = Buffer.from('{"name":"José","message":"Hello 世界"}');
      ...
      const duplicateFormCtx = new Context("POST", url, formHeaders, duplicateFormBody);
      ...
      const specialFormCtx = new Context("POST", url, formHeaders, specialFormBody);
      ...
      const malformedFormCtx = new Context("POST", url, formHeaders, malformedFormBody);
    ...
    it("should handle content-type edge cases", () => {
      ...
      const noContentTypeCtx = new Context("POST", url, noContentTypeHeaders, noContentTypeBody);
      ...
      const whitespaceCtx = new Context("POST", url, whitespaceHeaders, whitespaceBody);
      ...
      const multiParamCtx = new Context("POST", url, multiParamHeaders, multiParamBody);
      ...
      const caseInsensitiveCtx = new Context("POST", url, caseInsensitiveHeaders, caseInsensitiveBody);
    ...
    it("should handle security edge cases", () => {
      ...
      assert.equal(maliciousCtx.requestHeaders.get("x-header"), "value with spaces and special chars: <>&\"'");

      // Malicious query parameters
      const maliciousUrl = new URL("https://example.com/users?name=<script>alert('xss')</script>");
      const maliciousQueryCtx = new Context("GET", maliciousUrl, headers);
      assert.equal(maliciousQueryCtx.queryParams.get("name"), "<script>alert('xss')</script>");
    });
    ...
    it("should handle error recovery edge cases", async () => {
      ...
      const malformedJsonCtx = new Context("POST", url, headers, malformedJsonBody);
